...
// BackendMetrics contains all metrics for daemon backend operations.
...
// NewBackendMetrics creates a new BackendMetrics with the given namespace.
func NewBackendMetrics(namespace string) BackendMetrics {
	return BackendMetrics{
		PathsRequests: metrics.NewPromCounterFrom(prometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: "path",
			Name:      "requests_total",
			Help:      "The amount of path requests received.",
		}, []string{prom.LabelResult, prom.LabelDst}),
		PathsLatency: metrics.NewPromHistogramFrom(prometheus.HistogramOpts{
			Namespace: namespace,
			Subsystem: "path",
		...
		ASRequests: metrics.NewPromCounterFrom(prometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: "as_info",
			Name:      "requests_total",
			Help:      "The amount of AS requests received.",
		}, []string{prom.LabelResult}),
		ASLatency: metrics.NewPromHistogramFrom(prometheus.HistogramOpts{
			Namespace: namespace,
			Subsystem: "as_info",
			Name:      "request_duration_seconds",
		...
		InterfacesRequests: metrics.NewPromCounterFrom(prometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: "if_info",
			Name:      "requests_total",
			Help:      "The amount of interfaces requests received.",
		}, []string{prom.LabelResult}),
		InterfacesLatency: metrics.NewPromHistogramFrom(prometheus.HistogramOpts{
			Namespace: namespace,
			Subsystem: "if_info",
			Name:      "request_duration_seconds",
		...
		ServicesRequests: metrics.NewPromCounterFrom(prometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: "service_info",
			Name:      "requests_total",
			Help:      "The amount of services requests received.",
		}, []string{prom.LabelResult}),
		ServicesLatency: metrics.NewPromHistogramFrom(prometheus.HistogramOpts{
			Namespace: namespace,
			Subsystem: "service_info",
			Name:      "request_duration_seconds",
		...
		InterfaceDownNotifications: metrics.NewPromCounter(prom.SafeRegister(
		...
		InterfaceDownLatency: metrics.NewPromHistogramFrom(prometheus.HistogramOpts{
			Namespace: namespace,
...
func (c *ConnectorMetricsWrapper) Interfaces(ctx context.Context) (map[uint16]netip.AddrPort, error) {
	...
	if c.Metrics.InterfacesLatency != nil {
		c.Metrics.InterfacesLatency.With(prom.LabelResult, result).Observe(time.Since(start).Seconds())
	}
	return interfaces, unwrapMetricsError(err)
}

func (c *ConnectorMetricsWrapper) Paths(ctx context.Context, dst, src addr.IA, f PathReqFlags) ([]snet.Path, error) {
	start := time.Now()
	paths, err := c.Connector.Paths(ctx, dst, src, f)
	result := errToResult(err)
	if c.Metrics.PathsRequests != nil {
		c.Metrics.PathsRequests.With(prom.LabelResult, result, prom.LabelDst, dst.ISD().String()).Add(1)
	}
...
func (c *ConnectorMetricsWrapper) SVCInfo(ctx context.Context, svcTypes []addr.SVC) (map[addr.SVC][]string, error) {
	...
	if c.Metrics.ServicesLatency != nil {
		c.Metrics.ServicesLatency.With(prom.LabelResult, result).Observe(time.Since(start).Seconds())
	}
	return info, unwrapMetricsError(err)
}
...
func (c *ConnectorMetricsWrapper) RevNotification(ctx context.Context, revInfo *path_mgmt.RevInfo) error {
	...
	if c.Metrics.InterfaceDownNotifications != nil {
		c.Metrics.InterfaceDownNotifications.With(prom.LabelResult, result, prom.LabelSrc, "notification").Add(1)
	}
	if c.Metrics.InterfaceDownLatency != nil {
		c.Metrics.InterfaceDownLatency.With(prom.LabelResult, result).Observe(time.Since(start).Seconds())
	}
