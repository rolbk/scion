...
import (
	...
	drkey_daemon "github.com/scionproto/scion/pkg/daemon/drkey"
...
func (c *ConnectorBackend) svcInfo(ctx context.Context, svcTypes []addr.SVC) (map[addr.SVC][]string, error) {
	...
	// For now, we only support Control service
	// This could be extended based on svcTypes filter
	var services []string
	for _, h := range c.Topology.ControlServiceAddresses() {
		services = append(services, h.String())
	}

	// Map to the appropriate SVC type - assuming Control for now
	// You might need to adjust this based on your service type mappings
	if len(services) > 0 {
